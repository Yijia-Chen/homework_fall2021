...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        if self.agent_params['discrete']:
            ...
            for _ in range(n_actions):
                pass
        v_pi = torch.mean(qa_values, dim=-1)

        # DONE Calculate Q-Values
        q_vals = torch.gather(qa_values, 1, ac_na.long().unsqueeze(1)).squeeze(1)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            expl_bonus_unnormalized = self.exploration_model.forward_np(next_ob_no)
            expl_bonus = normalize(expl_bonus_unnormalized, np.mean(expl_bonus_unnormalized), np.std(expl_bonus_unnormalized))
